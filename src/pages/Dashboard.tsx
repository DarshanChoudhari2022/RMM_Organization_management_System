...
const SidebarItem = ({ id, label, icon: Icon, active, onClick }: any) => (
  ...
  >
    ...
    {active && (
      <motion.div
        layoutId="active-nav"
        className="absolute left-0 w-1 h-8 bg-[#1D4ED8] rounded-r-full"
      />
    )}
  </button>
);
  ...
  return (
    <div className="relative">
      <select
        ...
        className="appearance-none bg-white border border-gray-200 px-4 py-2 pr-8 rounded-xl text-sm font-bold text-[#0F172A]/90 outline-none focus:ring-2 focus:ring-[#1D4ED8]/20 cursor-pointer"
      >
        {years.map(y => <option key={y} value={y}>Year {y}</option>)}
      </select>
      <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-[#0F172A]/60 pointer-events-none" size={14} />
    </div>
  )
}
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <h2 className="text-xl font-display font-black text-[#0F172A]">Member Directory ({year})</h2>
        <button
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        {members?.length === 0 ? (
          <div className="p-10 text-center text-[#0F172A]/60 text-sm">No members found. Add one to get started.</div>
        ) : (
          <div className="overflow-x-auto">
            <div className="min-w-[800px]">
              {members?.map((member) => {
                ...
                return (
                  <div key={member.id} className="grid grid-cols-12 gap-4 p-4 border-b border-gray-50 items-center hover:bg-[#FDFBF7] transition-colors">
                    <div className="col-span-4">
                      ...
                      <div className="text-[10px] text-[#0F172A]/60">{member.role}</div>
                    </div>
                    <div className="col-span-3 text-sm text-[#0F172A]/80 font-mono">{member.phone}</div>
                    <div className="col-span-3 flex items-center gap-3">
                      {isEditing ? (
                        <div className="flex items-center gap-2">
                          <input
                            type="number"
                            className="w-20 px-2 py-1 border border-[#1D4ED8] rounded text-sm font-bold outline-none"
                        ...
                        <>
                          <span className="font-bold text-[#0F172A] w-12">₹{amount}</span>
                          <button
                            onClick={() => setEditingVargani({ id: member.id, amount })}
                            className="p-1 text-[#1D4ED8]/60 hover:text-[#1D4ED8] rounded transition-colors"
                            title="Edit Amount"
                          >
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Full Name</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Mobile Number</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Role</label>
                  <select
      ...
      <div className="pt-4 border-t border-gray-200">
        {!isAdding ? (
          <button
            ...
            className="text-[9px] font-black uppercase tracking-widest text-[#1D4ED8] hover:underline flex items-center gap-1"
          >
          ...
          <div className="bg-white p-3 rounded-lg border border-gray-200 space-y-3">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={handleManualAdd} className="flex-1 bg-[#1D4ED8] text-white py-1 text-[9px] font-black rounded uppercase">Save</button>
...
const TasksTab = ({ year }: { year: number }) => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <h2 className="text-xl font-display font-black text-[#0F172A]">Task Manager ({year})</h2>
        <button
          onClick={() => setIsAddOpen(true)}
          className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[#1D4ED8] text-white px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#1D4ED8]/90 transition-all shadow-lg shadow-[#1D4ED8]/20"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTasks.length === 0 ? (
          <div className="col-span-full py-20 text-center bg-[#F5F5F0] rounded-3xl border border-dashed border-gray-200">
            ...
            <button onClick={() => setIsAddOpen(true)} className="mt-4 text-[#1D4ED8] font-bold text-sm hover:underline">Create your first task</button>
          </div>
        ) : (
          filteredTasks.map(task => (
            <div key={task.id} className="bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-lg transition-all group">
              <div className="flex justify-between items-start mb-4">
                <div className="p-3 bg-orange-50 text-[#1D4ED8] rounded-xl">
                  <CheckCircle2 size={20} />
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => openEdit(task)}
                    className="p-2 text-[#1D4ED8]/60 hover:text-[#1D4ED8] hover:bg-orange-50 rounded-lg transition-colors"
                  >
                    <div className="w-4 h-4 border border-current rounded-sm flex items-center justify-center text-[10px] font-sans">✎</div>
                  </button>
              ...
              <h3 className="text-lg font-bold text-[#0F172A] mb-2">{task.title}</h3>
              <p className="text-sm text-[#0F172A]/70 line-clamp-2 mb-4">{task.description}</p>

              <div className="space-y-2 mb-6">
                <div className="flex items-center gap-2 text-xs text-[#0F172A]/70 font-medium">
                  <Calendar size={14} className="text-[#1D4ED8]" /> {task.date}
                </div>
                <div className="flex items-center gap-2 text-xs text-[#0F172A]/70 font-medium">
                  <Clock size={14} className="text-[#1D4ED8]" /> {task.time}
                </div>
                <div className="flex items-center gap-2 text-xs text-[#0F172A]/70 font-medium">
                  <MapPin size={14} className="text-[#1D4ED8]" /> {task.location}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  className={`w-full py-2 ${showAttendees === task.id ? "bg-[#0F172A] text-white" : "bg-gray-100 text-[#0F172A]/60"} rounded-xl font-bold text-[10px] uppercase tracking-widest transition-all`}
                >
        ...
        {isAddOpen && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Task Title</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Description</label>
                  <textarea
                    ...
                    className="w-full bg-[#F5F5F0] border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]/20 min-h-[80px]"
                    placeholder="Details about the task..."
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Date</label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Time</label>
                    <input
                      type="time"
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Location</label>
                  <input
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex items-center gap-4">
          <h2 className="text-xl font-display font-black text-[#0F172A]">Expenses ({year})</h2>
          <p className="text-xs text-[#0F172A]/60 font-bold uppercase tracking-wider ml-4 border-l pl-4 border-gray-100 hidden sm:block">
            Total for {year}: <span className="text-[#1D4ED8] text-lg ml-1">₹{totalSpent.toLocaleString()}</span>
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="hidden md:block bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        {filteredExpenses.length === 0 ? (
          <div className="p-10 text-center text-[#0F172A]/60 text-sm">No expenses recorded for {year}.</div>
        ) : (
          <div className="overflow-x-auto">
            <div className="min-w-[900px]">
              {filteredExpenses.map((ex) => (
                <div key={ex.id} className="grid grid-cols-12 gap-4 p-4 border-b border-gray-50 items-center hover:bg-[#FDFBF7] transition-colors">
                  <div className="col-span-2">
                    <div className="font-bold text-[#0F172A] line-clamp-2" title={ex.description}>{ex.description}</div>
                  </div>
                  <div className="col-span-2">
                    <div className="text-xs font-bold text-[#0F172A]/60">{ex.vendorName || "General"}</div>
                  </div>
                  <div className="col-span-1">
                    <span className={`text-[10px] font-black uppercase px-2 py-1 rounded ${ex.paidBy === 'Mandal' ? 'bg-[#1D4ED8]/10 text-[#1D4ED8]' : 'bg-purple-100 text-purple-700'}`}>
                      {ex.paidBy}
                    </span>
                  </div>
                  <div className="col-span-2">
                    <span className="bg-gray-100 text-[#0F172A]/80 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">{ex.category}</span>
                  </div>
                  <div className="col-span-1 text-[11px] text-[#0F172A]/70 font-mono">{ex.date}</div>
                  <div className="col-span-1 text-right">
                    <div className="font-black text-[#1D4ED8]">₹{ex.amount.toLocaleString()}</div>
                  </div>
                  ...
                  <div className="col-span-1 flex justify-end gap-2">
                    <button
                      onClick={() => openEdit(ex)}
                      className="p-2 text-[#1D4ED8]/60 hover:text-[#1D4ED8] hover:bg-orange-50 rounded-lg transition-colors border border-transparent hover:border-orange-100"
                    >
            ...
            <div key={ex.id} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="font-bold text-[#0F172A] text-lg leading-tight mb-1">{ex.description}</div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-[#0F172A]/40">Vendor: {ex.vendorName || "General"}</div>
                </div>
                <div className="text-right">
                  <div className="font-black text-[#1D4ED8] text-xl">₹{ex.amount.toLocaleString()}</div>
                  <div className="text-[10px] font-mono text-[#0F172A]/40">{ex.date}</div>
                </div>
              </div>

              <div className="flex flex-wrap gap-2 pt-2">
                <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded bg-[#F5F5F0] text-[#0F172A]/70`}>
                  {ex.category}
                </span>
                <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded ${ex.paidBy === 'Mandal' ? 'bg-[#1D4ED8]/10 text-[#1D4ED8]' : 'bg-purple-100 text-purple-700'}`}>
                  Paid By: {ex.paidBy}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                <div className="flex gap-2">
                  <button onClick={() => openEdit(ex)} className="p-2 bg-orange-50 text-[#1D4ED8] rounded-xl"><div className="w-4 h-4 border border-current rounded-sm flex items-center justify-center text-[10px] font-sans">✎</div></button>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Vendor Name</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Amount (₹)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Paid By</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Category</label>
                  <select
...
const SuppliersTab = () => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <h2 className="text-xl font-display font-black text-[#0F172A]">Supplier Contacts</h2>
        <button
          onClick={() => {
            setEditingSup(null);
            setIsAddOpen(true);
              ...
              {suppliers?.map((sup) => {
                ...
                return (
                  <div key={sup.id} className="grid grid-cols-12 gap-4 p-4 border-b border-gray-50 items-center hover:bg-[#FDFBF7] transition-colors">
                    <div className="col-span-2">
                      <div className="font-bold text-[#0F172A]">{sup.name}</div>
                      <span className="text-[10px] font-black uppercase tracking-wider text-[#1D4ED8]/60">{sup.category}</span>
                    </div>
                    <div className="col-span-2">
                      <div className="text-sm text-[#0F172A]/80 font-mono mb-1">{sup.contact}</div>
                      <a href={`tel:${sup.contact}`} className="text-[#1D4ED8] inline-flex items-center gap-1 text-[10px] font-bold"><Phone size={10} /> Call</a>
                    </div>
                    <div className="col-span-2">
                      <div className="flex justify-between text-[10px] font-black mb-1">
                        <span className="text-[#1D4ED8]">₹{sup.paid_amount?.toLocaleString() || 0}</span>
                        <span className="text-gray-400">/ ₹{sup.total_amount?.toLocaleString() || 0}</span>
                      </div>
                      <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-[#1D4ED8]" style={{ width: `${Math.min(progress, 100)}%` }}></div>
                      </div>
                    </div>
                    <div className="col-span-1">
                      {sup.is_confirmed ? (
                    ...
                    <div className="col-span-3 text-xs text-[#0F172A]/60 space-y-1">
                      {sup.terms && <div className="italic line-clamp-1">"Terms: {sup.terms}"</div>}
                      {sup.supplier_comment && <div className="text-[#1D4ED8] font-medium line-clamp-1">"Comment: {sup.supplier_comment}"</div>}
          ...
          suppliers?.map((sup) => {
            ...
            return (
              <div key={sup.id} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-bold text-[#0F172A] text-lg leading-tight mb-1">{sup.name}</div>
                    <div className="flex items-center gap-2">
                      <span className="bg-orange-50 text-[#1D4ED8] px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest">{sup.category}</span>
                      {sup.is_confirmed ? (
                        <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => openEdit(sup)} className="p-2 bg-orange-50 text-[#1D4ED8] rounded-xl"><div className="w-4 h-4 border border-current rounded-sm flex items-center justify-center text-[10px] font-sans">✎</div></button>
                ...
                <div className="space-y-3 py-2">
                  <div className="bg-gray-50 p-3 rounded-xl">
                    <div className="flex justify-between text-[11px] font-black mb-1">
                      <span className="text-[#0F172A]/60 uppercase tracking-widest">Payment Status</span>
                      <span className="text-[#1D4ED8]">₹{sup.paid_amount || 0} / ₹{sup.total_amount || 0}</span>
                    </div>
                  ...
                  <div className="bg-[#FDFBF7] p-4 rounded-xl space-y-3">
                    {sup.terms && (
                      <div className="text-xs text-[#0F172A]/70">
                        <span className="font-black text-[9px] uppercase tracking-widest block mb-1 text-[#1D4ED8]">Agreed Terms:</span>
                        "{sup.terms}"
                      </div>
                    )}
                    {sup.supplier_comment && (
                      <div className="text-xs text-[#0F172A]/70">
                        <span className="font-black text-[9px] uppercase tracking-widest block mb-1 text-[#1D4ED8]">Supplier Comment:</span>
                        "{sup.supplier_comment}"
                      </div>
                    )}
                    {sup.address && (
                      <div className="flex gap-2 items-start text-xs text-[#0F172A]/70">
                        <MapPin size={14} className="text-[#1D4ED8] shrink-0 mt-0.5" />
                        {sup.address}
                      </div>
                    )}
                    {sup.notes && (
                      <div className="text-xs text-[#0F172A]/60 italic border-l-2 border-[#1D4ED8]/20 pl-3">
                        "{sup.notes}"
                      </div>
                    )}
                  </div>
        ...
        {isAddOpen && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Name</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Contact No</label>
                  <input
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Total Amount (₹)</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Paid Amount (₹)</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Terms Agreed</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Shop Address</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Internal Notes</label>
                  <textarea
                    ...
                    className="w-full bg-[#F5F5F0] border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]/20 min-h-[60px]"
                    placeholder="Additional internal details..."
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 mb-1">Supplier Comment</label>
                  <textarea
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-xl shadow-gray-100/50">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-[#1D4ED8]/10 text-[#1D4ED8] rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Send size={28} />
          </div>
          <h2 className="text-2xl font-display font-black text-[#0F172A]">Create Invitation</h2>
          <p className="text-sm text-[#0F172A]/60 mt-2">Design a message and share it instantly.</p>
        </div>
        ...
        <div className="space-y-4">
          <input
            ...
            className="w-full bg-[#F5F5F0] border border-gray-200 rounded-xl p-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]/20"
          ...
          <div className="grid grid-cols-2 gap-4">
            <input
              ...
              className="w-full bg-[#F5F5F0] border border-gray-200 rounded-xl p-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]/20"
            />
            <input
              type="time"
          ...
          <textarea
            ...
            className="w-full bg-[#F5F5F0] border border-gray-200 rounded-xl p-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]/20 min-h-[100px]"
          ...
          <div className="bg-green-50 rounded-xl p-4 border border-green-100 mt-6">
            ...
            <pre className="whitespace-pre-wrap text-sm text-[#0F172A]/80 font-sans">{generateMsg()}</pre>
          </div>
...
const LetterheadTab = () => {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`RAHUL MITRA MANDAL_Letterhead_${new Date().getTime()}.pdf`);
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div className={`xl:col-span-4 space-y-6 ${activeMode === 'preview' ? 'hidden lg:block' : ''}`}>
          <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-xl space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar sticky top-4">
            ...
            <button
              onClick={downloadPDF}
              className="w-full flex items-center justify-center gap-2 bg-[#1D4ED8] text-white py-4 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-[#B94A15] transition-all shadow-lg shadow-[#1D4ED8]/20 mt-4"
            >
            ...
            <div className="w-full bg-gray-200/50 backdrop-blur-sm rounded-[40px] p-4 md:p-12 lg:p-16 border-2 border-dashed border-gray-300 flex justify-center items-start overflow-hidden min-h-[850px] shadow-inner mb-20">
              ...
              <div className="origin-top transition-transform bg-white shadow-2xl scale-[0.35] sm:scale-[0.5] md:scale-[0.7] lg:scale-[0.8] xl:scale-90 2xl:scale-100">
                ...
                >
                  ...
                  <div className="pt-12 px-16 pb-6">
                    <div className="flex justify-between items-start relative z-10">
                      <div className="flex gap-8 items-center">
                        <div className="relative">
                          <div className="absolute -inset-4 bg-[#1D4ED8]/10 rounded-full blur-xl"></div>
                      ...
                      <div className="text-right space-y-2 pt-4">
                        ...
                        <div className="h-1 w-full bg-[#1D4ED8] rounded-full"></div>
                      </div>
                    </div>
                  </div>

                  {/* Zigzag Header Border */}
                  <div className="w-full h-12 bg-[#1D4ED8] flex items-center mb-12 relative">
                    <div className="absolute inset-x-0 -bottom-6 flex overflow-hidden">
                      {[...Array(30)].map((_, i) => (
                        <div key={i} className="w-12 h-12 bg-[#1D4ED8] rounded-full shrink-0 -mt-6" />
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="px-20 flex-1 flex flex-col">
                    <div className="flex justify-between items-start mt-6">
                      <div className="space-y-2 text-xl">
                        <span className="block font-bold text-[#1D4ED8] text-sm uppercase tracking-widest">{data.toLabel}</span>
                        <p className="font-black text-[#0F172A]">{data.toName}</p>
                    ...
                    <div className="mt-14 flex justify-end">
                      <div className="text-right space-y-2 border-r-[12px] border-[#1D4ED8] pr-8 bg-gray-50/50 p-6 rounded-l-[32px] shadow-sm max-w-[400px]">
                        <span className="block font-black italic text-xs text-[#1D4ED8] uppercase tracking-[0.3em] mb-2">{data.applicantLabel}</span>
                        <p className="font-black text-3xl text-[#0F172A] leading-none mb-2">{data.applicant}</p>
                    ...
                    <div className="mt-20 text-center">
                      <div className="inline-block relative">
                        <h2 className="text-3xl font-black text-[#0F172A] px-8 py-2 relative z-10 bg-white border-2 border-[#1D4ED8] rounded-full shadow-lg">
                      ...
                      <div className="flex justify-end pr-12">
                        <div className="text-center space-y-16">
                          ...
                          <div className="space-y-3">
                            ...
                            <p className="font-black text-3xl text-[#1D4ED8] mb-1">{data.applicant}</p>
                  ...
                  <div className="h-20 bg-[#1D4ED8] flex items-center justify-center px-20 relative overflow-hidden mt-auto">
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center px-2 lg:px-0">
        <h2 className="text-xl font-display font-black text-[#0F172A]">Activity History</h2>
        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#0F172A]/40">
      ...
      <div className="bg-white lg:border border-gray-100 lg:rounded-3xl shadow-sm overflow-hidden border border-gray-100 rounded-2xl">
        {/* Desktop Header */}
        <div className="hidden lg:grid grid-cols-12 gap-4 p-5 bg-[#F5F5F0] text-[10px] font-black uppercase tracking-widest text-[#0F172A]/60 border-b border-gray-100">
            ...
            logs?.map((log) => (
              <div key={log.id} className="lg:grid lg:grid-cols-12 gap-4 p-4 lg:p-5 items-center hover:bg-gray-50/50 transition-colors bg-white group">
                ...
                <div className="lg:col-span-2 mb-2 lg:mb-0 flex lg:flex-col justify-between lg:justify-center whitespace-nowrap">
                  <div className="text-[11px] lg:text-xs font-bold text-[#0F172A]">
                    {new Date(log.created_at).toLocaleDateString('mr-IN')}
                  </div>
                  <div className="text-[10px] text-[#0F172A]/50 font-mono">
                ...
                <div className="lg:col-span-3 mb-2 lg:mb-0">
                  <div className="text-[10px] font-black uppercase tracking-wider text-[#1D4ED8] bg-[#1D4ED8]/5 lg:bg-[#1D4ED8]/10 px-2 py-1 rounded border border-[#1D4ED8]/10 truncate max-w-full inline-block align-middle" title={log.user_name || "System"}>
                ...
                <div className="lg:col-span-2 mb-1 lg:mb-0">
                  <div className="text-[10px] lg:text-xs font-black uppercase tracking-wider text-[#0F172A] whitespace-nowrap">
                    {log.action}
                  </div>
                </div>
                ...
                <div className="lg:col-span-5">
                  <p className="text-[11px] lg:text-xs text-[#0F172A]/70 leading-relaxed bg-gray-50 lg:bg-transparent p-2 lg:p-0 rounded-lg lg:rounded-none">
                    <span className="lg:hidden font-black text-[9px] uppercase tracking-widest text-[#0F172A]/40 block mb-1">Details:</span>
